...
class Formulaire(FlaskForm):
    ...
    niveau = SelectField("Niveau d'étude", choices=[("","")]+doSqlWithoutEffect("SELECT IDStatutSColaire, NiveauScolaire FROM StatutScolaire"), validators=[validators.InputRequired(message="Veuillez renseigner votre niveau d'étude")])
    alimentsMatin = []
    alimentsSoir = []
...
class FormAliment(FlaskForm):
    groupe = SelectField("groupe", choices=[("Non Selectionné","Non Selectionné")]+doSqlWithoutEffect(f"SELECT alim_grp_code, alim_grp_nom_fr FROM GroupeAliment"))
